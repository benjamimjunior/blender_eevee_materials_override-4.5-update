...
class EeveeMaterialsOverride:
    ...
    def restore_objects(cls, objects):
        ...
        for obj in objects:
            backups = list(obj.eevee_materials_override_mat_backup)
            for i, material_backup in enumerate(backups):
                if i < len(obj.material_slots):
                    obj.material_slots[i].material = material_backup.material
            obj.eevee_materials_override_mat_backup.clear()

    @classmethod
    def change_mode(cls, scene_data, mode: bool):
        # change override mode (on - off)
        # Find existing nodegroup without creating a new one
        override_nodegroup = next(
            (ng for ng in scene_data.node_groups if cls._overrider_id in ng), None
        )
        if not override_nodegroup:
            return
    ...
    def clay_nodegroup(cls, node_groups: list):
        ...
        if not nodegroup:
            ...
            glossy_bsdf.inputs['Roughness'].default_value = 0.45
            # ShaderNodeMixRGB was removed in Blender 4.0; use ShaderNodeMix with data_type='RGBA'
            ...
            nodegroup.links.new(layer_weight.outputs[1], mix.inputs['Factor'])
